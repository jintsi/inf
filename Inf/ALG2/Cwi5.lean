...
def Zad2.F (R) [Semiring R] : Dual R (Fin 3 → R) where
...
def Zad2d.inner (R) [Add R] [Mul R] [NatCast R] (x y : Fin 3 → R) :=
...
theorem Zad4a [CommSemiring R] : Nat.factorial i.val • ⇑((degreeLT.basis R n).dualBasis i)
...
noncomputable def Zad4.F : Dual ℝ ℝ[X]_3 where
...
open Zad4 in
theorem Zad4b : ⇑((degreeLT.basis ℝ 3).dualBasis.repr F) = ![1, 2⁻¹, 3⁻¹] := by
  ext i; simp [F]; fin_cases i <;> norm_num

/-- Basis `B = ![!![1, 0; 0, 0], !![1, 1; 0, 0], !![1, 1; 1, 0], !![1, 1; 1, 1]]` -/
noncomputable def Zad5.B : Basis (Fin 4) ℝ (Matrix (Fin 2) (Fin 2) ℝ) :=
...
theorem Zad6 [CommSemiring R] [AddCommGroup M] [Module R M] [DecidableEq ι] [Fintype ι]
...
theorem ZadD1 [CommSemiring R] [AddCommGroup M₁] [AddCommGroup M₂] [Module R M₁] [Module R M₂]
...
theorem ZadD5 [Field K] [AddCommGroup V] [Module K V] {f g : Dual K V} (h : f.ker = g.ker) :
    g ∈ K ∙ f := by
