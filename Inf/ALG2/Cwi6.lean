...
theorem Zad6_1 (hA : A ∈ unitaryGroup n α) (hB : B ∈ unitaryGroup n α) :
    A * B ∈ unitaryGroup n α := Submonoid.mul_mem _ hA hB

theorem Zad6_2 [CommRing R] [NoZeroDivisors R] [LinearOrder R] [Nontrivial R] [IsOrderedRing R]
...
theorem Zad6_3 {A : Matrix n n K} (hA : A.IsHermitian) :
    spectrum K A ⊆ Set.range RCLike.ofReal := hA.spectrum_eq_image_range ▸ Set.image_subset_range _ _

theorem Zad6_4 {A : Matrix n n ℂ} (hA : A.IsHermitian) (hz : Complex.im z ≠ 0) :
  ...
  apply Set.subset_iff_notMem.mp (Zad6_3 hA.neg); simp_rw [Set.mem_range, not_exists, ← ne_eq]
  intro x; apply_fun Complex.im; simpa [eq_comm]

theorem Zad6_5 {A B : Matrix n n K} (hA : A.IsHermitian) :
...
theorem Zad6_6 {A : Matrix n n K} (hA : A.IsHermitian) (h : IsNilpotent A) : A = 0 := by
...
theorem Zad6_8 {A : Matrix n n K} (hA : A.IsHermitian) : ∃ a : ℝ, (A + algebraMap _ _ a).PosDef := by
  by_cases! IsEmpty n
...
theorem Zad6_9 {A B : Matrix n n K} (hA : A.IsHermitian) (hB : B.IsHermitian) :
...
theorem Zad6_11 : A ∈ orthogonalGroup (Fin 2) ℝ ↔ ∃ θ,
...
theorem Zad6_D1 [Nonempty n] (hA : A ∈ orthogonalGroup n K) (hB : B ∈ orthogonalGroup n K) :
    A ^ 2 - B ^ 2 ≠ A * B := by
  apply_fun (Aᵀ * · * Bᵀ)
...
alias Zad6_D3 := posDef_gram_of_linearIndependent
