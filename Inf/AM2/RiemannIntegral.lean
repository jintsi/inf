...
abbrev interval : Box Unit where
  lower _ := a
  upper _ := b
  lower_lt_upper _ := hab

theorem mem_interval (i : Unit → ℝ) : i ∈ interval hab ↔ i () ∈ Set.Ioc a b := Unique.forall_iff

/-- An `i`th subinterval of `intervalPartition hab n`. -/
noncomputable def intervalBox (i : ℕ) : Box Unit :=
  interval (a := a + (b - a) * i / n) (b := a + (b - a) * (i + 1) / n) (by gcongr; simp)

include hab hn in
lemma intervalBox_bounds (hi : i < n) :
    a ≤ a + (b - a) * i / n ∧ a + (b - a) * (i + 1) / n ≤ b := by
  use (by bound); field_simp; rw [← le_sub_iff_add_le', ← sub_mul]; gcongr; norm_cast

open Classical in
/-- A partition of `Set.Ioc a b` into `n` intervals of length `(b - a) / n`. -/
noncomputable def intervalPartition : Prepartition (interval hab) where
  boxes := (Finset.range n).image (intervalBox hab hn)
  le_of_mem' := by
    simpa [intervalBox, fun I J => (Box.le_TFAE I J).out 0 3, Pi.le_def, -le_add_iff_nonneg_right]
      using fun i => intervalBox_bounds hab hn
  pairwiseDisjoint := by
...
noncomputable def intervalPartition_index (a b : ℝ) (n : ℕ) (x : Unit → ℝ) :=
  ⌈(x () - a) * n / (b - a)⌉₊ - 1

theorem mem_intervalBox_index (x : Unit → ℝ) (hx : a < x ()) :
...
include hab hn in
lemma intervalPartition_index_lt (x : Unit → ℝ) (hx : a < x () ∧ x () ≤ b) :
...
theorem intervalPartition_isPartition : (intervalPartition hab hn).IsPartition := by
...
noncomputable def taggedIntervalPartition' (t : ℕ → ℝ)
    (ht : ∀ i < n, t i ∈ Set.Icc a b) : TaggedPrepartition (interval hab) where
  ...
  tag_mem_Icc := by
    ...
    case isFalse h =>
      simp only [not_or, not_le, not_lt] at h
      rw [Box.Icc_eq_pi, Set.mem_univ_pi, Unique.forall_iff]
      apply ht; apply intervalPartition_index_lt hab hn u h

/-- `taggedIntervalPartition'` with stronger prerequisites to make it a Henstock partition. -/
noncomputable abbrev taggedIntervalPartition (t : ℕ → ℝ)
...
theorem intervalPartition_isHenstock (t : ℕ → ℝ)
  ...
  rw [add_le_iff_nonpos_right, eq_iff_iff, iff_false, not_or, not_lt, not_le]
  exact ⟨by bound, (intervalBox_bounds hab hn hi).right⟩

open Classical in
theorem intervalPartition_isSubordinate (t : ℕ → ℝ)
    (ht : ∀ i : ℕ, t i ∈ Set.Icc (a + (b - a) * i / n) (a + (b - a) * (i + 1) / n))
...
theorem tendsto_intervalPartition_toFilteriUnion (t : ℕ → ℕ → ℝ)
...
theorem ContinuousOn.tendsto_integralSum_intervalIntegral {f : ℝ → E}
    ...
    (l.toFilteriUnion (interval hab) ⊤) (𝓝 (∫ x in a..b, f x)) := by
...
open Classical in
include hab in
theorem tendsto_sum_intervalIntegral {f : ℝ → E} (hc : ContinuousOn f (Set.Icc a b)) {t : ℕ → ℕ → ℝ}
    (ht : ∀ n : ℕ, n ≠ 0 → ∀ i : ℕ, t n i ∈ Set.Icc (a + (b - a) * i / n) (a + (b - a) * (i + 1) / n)) :
  ...
  · rw [ite_cond_eq_false, index_intervalBox]; simp_rw [intervalBox]
    rw [add_le_iff_nonpos_right, eq_iff_iff, iff_false, not_or, not_lt, not_le]
    exact ⟨by bound, (intervalBox_bounds hab n.succ_ne_zero (by simpa using hi)).right⟩
  · intro i hi j hj h
    apply_fun fun J => intervalPartition_index a b (n + 1) J.upper at h
    rw [index_intervalBox, index_intervalBox] at h; exact h

include hab in
theorem tendsto_sum_left_intervalIntegral {f : ℝ → E} (hc : ContinuousOn f (Set.Icc a b)) :
    Tendsto (fun n : ℕ => ((b - a) / n) • ∑ i ∈ Finset.range n, f (a + (b - a) * i / n))
    atTop (𝓝 (∫ x in a..b, f x)) :=
  tendsto_sum_intervalIntegral hab hc fun n hn i => Set.left_mem_Icc.mpr (by field_simp; grind)

include hab in
theorem tendsto_sum_right_intervalIntegral {f : ℝ → E} (hc : ContinuousOn f (Set.Icc a b)) :
    Tendsto (fun n : ℕ => ((b - a) / n) • ∑ i ∈ Finset.range n, f (a + (b - a) * (i + 1) / n))
    atTop (𝓝 (∫ x in a..b, f x)) :=
  tendsto_sum_intervalIntegral hab hc fun n hn i => Set.right_mem_Icc.mpr (by field_simp; grind)

include hab in
theorem tendsto_sum_midpoint_intervalIntegral {f : ℝ → E} (hc : ContinuousOn f (Set.Icc a b)) :
    Tendsto (fun n : ℕ => ((b - a) / n) • ∑ i ∈ Finset.range n, f (a + (b - a) * (i + 2⁻¹) / n))
    atTop (𝓝 (∫ x in a..b, f x)) :=
  tendsto_sum_intervalIntegral hab hc fun n hn i => Set.mem_Icc.mpr (by field_simp; grind)

theorem tendsto_sum_left_intervalIntegral_zero {a : ℝ} (ha : 0 < a) {f : ℝ → E} (hc : ContinuousOn f (Set.Icc 0 a)) :
    Tendsto (fun n : ℕ => (a / n) • ∑ i ∈ Finset.range n, f (a * i / n)) atTop (𝓝 (∫ x in 0..a, f x)) := by
  convert tendsto_sum_left_intervalIntegral ha hc using 5 <;> simp

theorem tendsto_sum_right_intervalIntegral_zero {a : ℝ} (ha : 0 < a) {f : ℝ → E} (hc : ContinuousOn f (Set.Icc 0 a)) :
    Tendsto (fun n : ℕ => (a / n) • ∑ i ∈ Finset.range n, f (a * (i + 1) / n)) atTop (𝓝 (∫ x in 0..a, f x)) := by
  convert tendsto_sum_right_intervalIntegral ha hc using 5 <;> simp

theorem tendsto_sum_right_intervalIntegral_zero_one {f : ℝ → E} (hc : ContinuousOn f (Set.Icc 0 1)) :
