...
theorem Zad1a : ¬IntervalIntegrable (fun x => (x * log x)⁻¹) MeasureTheory.volume 1 √(exp 1) := by
  ...
  · refine tendsto_abs_atBot_atTop.comp (tendsto_log_nhdsNE_zero.comp ?_)
...
theorem Zad1b : ∫ x in Set.Iic (-2), (1 - x ^ 2)⁻¹ = -(log 3 / 2) := by
  ...
  · filter_upwards [eventually_ge_atTop 2] with x hx; rw [← log_div] <;> grind
  exact ((((tendsto_sub_atTop 1).const_div_atTop 2).const_add_zero 1).log_one).zero_div_const 2
...
theorem Zad1c : ∫ x in Set.Ioi 0, exp (-x) * sin (x / 2) = 2 / 5 := by
  ...
  · intro x hx; convert DifferentiableAt.hasDerivAt ?_ using 1
  ...
  · refine ((tendsto_neg_atTop.rexp_atBot.const_mul_zero (2 / 5)).zero_mul_isBoundedUnder_le ?_).neg_zero
    simp [IsBoundedUnder, IsBounded, -neg_add_rev]; use 3, 0; intro x _
...
theorem Zad1d : ∫ (x : ℝ), 8 * x / (x ^ 4 + 4 * x ^ 2 + 3) = 0 := by
  have h : ∫ x in Set.Ioi 0, 8 * x / (x ^ 4 + 4 * x ^ 2 + 3) = 0 - (2 * log (1 - 2 / (0 ^ 2 + 3))) := by
    ...
    · exact ((((tendsto_pow_atTop two_ne_zero).atTop_add_const 3).const_div_atTop 2).const_sub_zero 1
        ).log_one.const_mul_zero 2
  ...
  rw [← integral_Iic_add_Ioi, h, h2, add_sub_cancel] <;>
    (apply MeasureTheory.Integrable.of_integral_ne_zero; norm_num [h, h2])
...
theorem Zad1f : ∫ x in 0..1, (√(x - x ^ 2))⁻¹ = π := calc ∫ x in 0..1, (√(x - x ^ 2))⁻¹
  _ = 2 * ∫ x in 0..1, (2 * √(x - x ^ 2))⁻¹ := by simp_rw [mul_inv, integral_const_mul]; ring_nf
  _ = 2 * ∫ x in 0..1, (√(2 ^ 2 * (x - x ^ 2)))⁻¹ := by simp
  _ = 2 * ∫ x in 0..1, (√(1 - (2 * x - 1) ^ 2))⁻¹ := by ring_nf
  _ = ∫ x in -1..1, (√(1 - x ^ 2))⁻¹ := by
    rw [mul_integral_comp_mul_sub (f := fun x => (√(1 - x ^ 2))⁻¹)]; norm_num
  _ = π := by simp
